# Testing VDK Module Gemfile Sources

## Description

This guide explains how to vary the gemsources and gem versions in the Gemfile of your vdk module using the following environment variables:
...
- Create a local directory for testing:
  ...
  mkdir -p ~/vdk_gemfile_testing/{new_module_tests,update_module_tests}
  cd ~/vdk_gemfile_testing
  ```
...
- `vdk new module` - Can a new module swap in different gemsources and puppet/facter versions?
- `vdk update` - Will an existing module convert to the new Gemfile ok?

### `vdk new module` scenarios
...
rm -rf ~/vdk_gemfile_testing/new_module_tests/test_default_gems 
vdk new module test_default_gems --skip-interview --template-ref="${TEMPLATE_URL}"

cd ~/vdk_gemfile_testing/new_module_tests/test_default_gems
...
# Create new module with enhanced template (local filesystem)
rm -rf ~/vdk_gemfile_testing/new_module_tests/test_puppetcore_gems 
vdk new module test_puppetcore_gems --skip-interview --template-url="${TEMPLATE_URL}"

cd test_puppetcore_gems 

# clean again before bundle install and set authentication
source ~/vdk_gemfile_testing/clean_environment.sh
source ~/vdk_gemfile_testing/set_puppetcore_authentication.sh
...
vdk new module test_git_gems --skip-interview --template-url="${TEMPLATE_URL}"

cd test_git_gems

# remove Gemfile and vendor because the vdk creates the module with a different lock file
...
# Create module with default VDK template first
# vdk new module test_update_module --skip-interview
...
# rm -rf ~/vdk_gemfile_testing
```

## Appendix

### Why can't VDK use your bundled environment?

VDK is designed as a self-contained tool with its own isolated Ruby and gem environment for consistency and reliability. In other words, the VDK has its own internal Puppet installation and doesn't automatically use the bundled Puppet version.  For example, when you run `vdk validate`, it will use VDK's internal Puppet version (e.g., 8.10.0) instead of your bundled version (e.g., 8.14.0 from puppetcore).
