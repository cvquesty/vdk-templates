...
[![Code Owners](https://img.shields.io/badge/owners-DevX--team-blue)](https://github.com/puppetlabs/vdk-templates/blob/main/CODEOWNERS)
![ci](https://github.com/puppetlabs/vdk-templates/actions/workflows/ci.yml/badge.svg)

The vdk-templates repository contains the default templates (or blueprints) used by the [Puppet Development Kit](https://github.com/voxpupuli/vdk) to create, convert, and update modules.

The following table shows the directories where different types of template are stored.

|                   |               |
| :-----------------|:--------------|
| `moduleroot`      | Stores templates that are deployed on vdk new module, vdk convert and vdk update commands. Use the templates in this directory to enforce a common boilerplate for central files. |
...
Use this README to understand the purpose, basic usage, and common configurable values for VDK templates. For further information about VDK, refer to our [documentation site](https://www.puppet.com/docs/vdk/2.x/vdk.html).

## Table of contents

* [Introduction](#vdk-templates)
* [How templates are used by VDK](#how-templates-are-used-by-vdk)
...
* [VDK default config values](#vdk-default-config-values)
* [Further Notes](#further-notes)

## How templates are used by VDK

When you use VDK to build, convert, or update a module, VDK accesses the relevant template in the vdk-templates repository to retrieve default “instructions” on how to build the module. Those instructions are applied to the local repo in which the command was run.

When configuring the module, VDK reads a set of default settings in `.config_defaults.yml` and merges it with the configuration settings in `.sync.yml`.

The result of this merge creates a hidden hash containing the final configuration of the module that VDK applies. Top-level keys of this resulting hash correspond to target files. A global configuration is merged with the configuration hash for a particular target file. This allows module developers to override or amend the configuration by adding new values through `.sync.yml`. For example, a knockout prefix (---) can be applied to keys in `.sync.yml` to remove default configuration values.

Target files are created by rendering a corresponding template which refers its configuration via the `@configs` hash.

You can configure VDK to make files executable if the file is executable in the template. To do this, include `manage_execute_permissions: true` either in the target file or globally via the common config key.

## Basic Usage

By default VDK uses the templates within this repository to render files for use within a module. To use a third-party template, pass its URL using the `–-template-url` flag as shown in the following example:

```bash
vdk convert --template-url https://github.com/puppetlabs/vdk-templates
```

VDK will default to the `main` branch when looking at the repository. You can specify other branches by using the `--template-ref`, like this:

```bash
vdk convert --template-url https://github.com/puppetlabs/vdk-templates --template-ref branch_name
```

Note: Commands run on VDK will use the last specified template repository.

When you have specified the template repository, you can get started with creating a module, if you don't already have one. Once your module is created, you can customize VDK-templates by editing the `.sync.yml` to override the default configuration, and then running `VDK update` on the module so that the new settings are applied.

For more information on basic usage and more detailed description of VDK in action please refer to [VDK documentation](https://github.com/voxpupuli/vdk/blob/main/README.md), where you can find detailed instructions on how to create, convert and update modules, as well as other useful commands.
...
Here we will provide some examples of how you can use `.sync.yml`. For a more comprehensive list of default settings, check the [VDK default config values](#vdk-default-config-values) section.
...
In the following example, the `unmanaged` and `delete` keys are used to specify that, regardless of the default setting defined in `config_defaults.yml`, VDK should leave the `ci.yml` file unmodified and delete `travis.yml`.
...
To add a custom internal `openvox-lint` plugin served from an internal Rubygems source, add
an entry similar to the following in `.sync.yml` file and run `vdk update`.

```yaml
Gemfile:
  optional:
    ':development':
      - gem: 'openvox-lint-my_awesome_custom_module'
        version: '>= 2.0'
...
## VDK default config values

The following is a description and explanation of each of the keys within `.config_defaults.yml`. This will help clarify the default settings that are applied by VDK. 
...
>A .vdkignore file in your repo allows you to specify files to ignore when building a module package with `vdk build`.

| Key               | Description   |
| :-----------------|:--------------|
| required          | The default list of files or paths for VDK to ignore when building a module package.
| paths             | Defines additional files or paths for VDK to ignore when building a module package.
...
|default\_disabled\_lint\_checks| Defines any checks that are to be disabled by default when running lint checks. As default we disable the `--relative` lint check, which compares the module layout relative to the module root. _Does affect **.openvox-lint.rc**._ |
|extra\_disabled\_lint\_checks| Defines any checks that are to be disabled as extras when running lint checks. No defaults are defined for this configuration. _Does affect **.openvox-lint.rc**._ |
...
|linter\_options| An array of options to be passed into linter config. _Does affect **.openvox-lint.rc**._ |
|linter\_fail\_on\_warnings| A boolean indicating if the linter should exit non-zero on warnings as well as failures. _Does affect **.openvox-lint.rc**._ |
...
Please note that the early version of this template contained only a 'moduleroot' directory, and did not have a 'moduleroot\_init'. The VDK 'vdk new module' command will still work with templates that only have 'moduleroot', however the 'vdk convert' command will fail if the template does not have a 'moduleroot_init' directory present. To remedy this please use the up to date version of the template.
...
To keep confusion to a minimum, the version we use is always on a `vdk-templates-v1` branch.
This way we can update (`git fetch`/`git push`) forked repositories with no prejudice, test out the changes, and only then update the `vdk-templates-v1` branch.
That said, the branches used in vdk-templates should only contain upstream code and changes already in an upstream PR to minimize the diff we're carrying.
...
* [kvrhdn/gha-buildevents](https://github.com/kvrhdn/gha-buildevents) ➡️ [puppetlabs/kvrhdn-gha-buildevents](https://github.com/puppetlabs/kvrhdn-gha-buildevents/tree/vdk-templates-v1)
* [Gamesight/slack-workflow-status](https://github.com/Gamesight/slack-workflow-status) ➡️ [puppetlabs/Gamesight-slack-workflow-status](https://github.com/puppetlabs/Gamesight-slack-workflow-status/tree/vdk-templates-v1)
